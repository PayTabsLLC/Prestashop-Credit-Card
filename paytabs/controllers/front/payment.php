...
class PaytabsPaymentModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	$request_param = array(
	                ...
	                'return_url' => Context::getContext()->link->getModuleLink('paytabs', 'validation')
	            );

		$request_string = http_build_query($request_param);
		
		$response_data = $this->module->sendRequest($gateway_url . '/apiv2/create_pay_page', $request_string);
		...
		if(isset($object->payment_url) && $object->payment_url != ''){
			...
			$error = $this->module->getErrorMessage($object->error_code);
			$this->context->smarty->assign(array(
				'error' => $error,
